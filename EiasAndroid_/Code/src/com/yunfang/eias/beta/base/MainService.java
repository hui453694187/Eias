package com.yunfang.eias.beta.base;
...
import com.yunfang.eias.beta.http.task.PushCoordinateTask;
import com.yunfang.eias.beta.http.task.SubmitTaskInfoTask;
import com.yunfang.eias.beta.http.task.UploadFileTask;
import com.yunfang.eias.beta.logic.DataLogOperator;
import com.yunfang.eias.beta.logic.TaskOperator;
...
public class MainService extends BaseBackgroundService {
			...
			while (uploadTasks.size() > 0) {
				...
				try {
					...
					if (getInfo.Success && getInfo.Data.ID > 0) {
						...
						/*
						 * //直接上传图片到资源库 ResultInfo<Map<String, String>>
						 * fileUploadSuccess = new ResultInfo<Map<String,
						 * String>>(); UploadFile2ResourceTask taskHttpRes = new
						 * UploadFile2ResourceTask
						 * (EIASApplication.getCurrentUser
						 * ().ResourceLibraryDomainName);
						 */
						...
						if (additional) {
							...
							if (ListUtil.hasData(taskDataItems.Data)) {
								...
								/*
								 * // 修改上传逻辑,直接同步资源库 fileUploadSuccess =
								 * taskHttpRes
								 * .request2(EIASApplication.getCurrentUser(),
							...
							if (ListUtil.hasData(taskDataItems.Data)) {
								/*
								 * fileUploadSuccess =
								 * taskHttpRes.request2(EIASApplication
								 * .getCurrentUser(), getInfo.Data,
								 * taskDataItems.Data, additional);
								 */
								fileUploadSuccess = taskHttp.request(EIASApplication.getCurrentUser(), getInfo.Data, taskDataItems.Data, additional);
							}
						}
						// 图片上传成功或者没有文件
						if (fileUploadSuccess.Success || !ListUtil.hasData(taskDataItems.Data)) {// &&!fileUploadSuccess.Data.isEmpty()
							/*
							 * // 资源上成功， 替换 分类项子项Value
							...
							showNotification(tempTask.ID, tempTask.TaskNum, "图片上传失败:请检查网络", "地址：" + tempTask.TargetAddress, tempTask.TaskNum + "提交失败");
							TaskOperator.saveTaskUploadStatus(TaskUploadStatusEnum.SubmitFailure, tempTask.TaskNum);
							tempTask.UploadStatusEnum = TaskUploadStatusEnum.SubmitFailure;
							// 是否需要重新提交
							isResubmit = true;
							/*
							 * 图片上传资源库失败， 删除失败任务的所有资源引用
							 * TaskOperator.deleteRelationResource(//
							 * EIASApplication.getCurrentUser(),// getInfo.Data,
							 * fileUploadSuccess.Data);
							 */
						}
					}
				} catch (Exception e) {
					DataLogWorker.createDataLog(EIASApplication.getCurrentUser(), tempTask == null ? "任务提交时从任务的队列池中获取任务出错" : "任务提交失败，后台服务调用失败，任务编号为：" + tempTask.TaskNum, OperatorTypeEnum.TaskSubmit,LogType.Exection);
	...
	private TaskInfo setDataItemValue(TaskInfo taskInfo, Map<String, String> resIdMap) {
		...
		for (String key : keySet) {
			...
			for (TaskCategoryInfo categories : taskInfo.Categories) {
				if (categories.BaseCategoryID == BaseCategoryID) {
