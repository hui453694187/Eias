...
import com.yunfang.eias.beta.ui.Adapter.DialogResultListAdapter;
...
public class AppHeader {
	 ...
	 * 
	 * @author kevin
	 * @date 2015-10-20 下午12:55:22
	 * @Description: 标题的点击事件
	 * @param listener
	 */
	public void setTitleOnClickListener(OnClickListener listener) {
		home_txt_title.setOnClickListener(listener);
	}
	...
	public void downloadTipsDialog(String msg) {
		...
		if (EIASApplication.IsNetworking) {
			try {
				// 如果版本不一致就下载最新的
				// 检查服务端版本名是否为空
				boolean isNull = EIASApplication.versionInfo.ServerVersionName != null
						&& EIASApplication.versionInfo.ServerVersionName.trim()
								.length() > 0;
			
				// 本地版本名于服务端版本是否一致， 不能为空， 不可以为null字符串
				if (TextUtils
						.isEmpty(EIASApplication.versionInfo.ServerVersionName)
						|| "null"
								.equals(EIASApplication.versionInfo.ServerVersionName)) {
					return;
				}
				...
				boolean euqalsVersion = !EIASApplication.versionInfo.LocalVersionName
						.equals(EIASApplication.versionInfo.ServerVersionName);
				if (isNull && euqalsVersion) {
					...
					new AlertDialog.Builder(currentContext)
							.setTitle(
									"确认(当前版本:"
											+ EIASApplication.versionInfo.LocalVersionName
											+ ")")
							.setMessage(
									"发现新版本["
											+ EIASApplication.versionInfo.ServerVersionName
											+ "]是否需要更新?")
	...
	/***
	 * @author kevin
	 * @date 2015-11-4 下午5:42:02
	 * @Description: 判断父容器，是否存在。
	 * @return boolean 返回类型
	 */
