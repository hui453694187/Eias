...
import com.yunfang.eias.beta.enumObj.TaskMenuEnum;
import com.yunfang.eias.beta.enumObj.TaskStatus;
import com.yunfang.eias.beta.http.task.GetFinishInworkReportTask;
import com.yunfang.eias.beta.logic.DataLogOperator;
import com.yunfang.eias.beta.logic.HomeOperator;
...
import com.yunfang.eias.beta.ui.Adapter.TaskListViewAdapter;
import com.yunfang.eias.beta.view.PullToRefreshLayout;
import com.yunfang.eias.beta.view.PullToRefreshLayout.OnRefreshListener;
import com.yunfang.eias.beta.viewmodel.TaskListViewModel;
...
public class TaskListFragment extends BaseWorkerFragment {
		...
		case TASK_GETTASKINFOES:
			...
			if (viewModel.currentIndex == 1) {
				...
				if (!EIASApplication.IsOffline) {// 离线时不需要前后台同时加载
					ResultInfo<Integer> temp = TaskDataWorker
							.createdByUserTaskTotal(viewModel.currentUser);
	...
	private void setCountTitle() {
		int count = (int) titleTextView.getTag();
			...
			public void onScrollStateChanged(AbsListView view, int scrollState) {
				if (viewModel.taskStatus != TaskStatus.Submiting) {
					...
					case OnScrollListener.SCROLL_STATE_IDLE:// 当屏幕停止滚动时
						...
						if (viewModel.listItemCurrentPosition == vc - 3) {// 滚动到最后一项
							int temp = 0;
							if (EIASApplication.IsOffline) {
								temp = (Integer) (viewModel.localTotal / viewModel.pageSize);
					...
					case OnScrollListener.SCROLL_STATE_TOUCH_SCROLL:// 当屏幕滚动且用户使用的触碰或手指还在屏幕上时
						/*int lastViesible=view
						.getLastVisiblePosition();
						int vcs = view.getCount();
						if(lastViesible==vcs-4){
	...
	private void synchroReportInfo() {
		if (EIASApplication.IsNetworking && !EIASApplication.IsOffline
				&& viewModel.currentIndex == 1 && serach_editText.length() <= 0
				&& viewModel.taskStatus == TaskStatus.Done) {
			try {
				// 获取最后的报告日期同步时间
