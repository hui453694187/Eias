...
import com.yunfang.framework.utils.PinYin4j;
...
//					ArrayList<T> list = new ArrayList<T>();//无
				...
				for (int i = 0; i < count; i++) {
					...
					Iterator<String> iterator= pinyinSet.iterator();//支持多音字
				...
				List<T> list=SetToList(newValues);//转成List
				results.values = list;
				results.count = list.size();
			}
			return results;
	...
	public <T extends Object> Set<T> ListToSet(List<T> tList) {   
		Set<T> tSet = new HashSet<T>(tList);   
		//TODO 具体实现看需求转换成不同的Set的子类。   
		return tSet;   
	}
	@SuppressWarnings("hiding")
	public <T extends Object> List<T> SetToList(Set<T> oSet) {   
